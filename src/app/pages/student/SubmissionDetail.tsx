...
export default function StudentSubmissionDetail() {
  ...
  if (loading) return <PortalPage className="max-w-5xl"><div className="h-72 rounded-[28px] bg-slate-100 animate-pulse dark:bg-slate-800/70" /></PortalPage>;
  if (error) return <PortalPage className="max-w-5xl"><div className="rounded-[24px] border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 dark:border-rose-400/25 dark:bg-rose-500/15 dark:text-rose-200">{error}</div></PortalPage>;

  if (!submission) {
    return <PortalPage className="max-w-5xl space-y-4">
      <button onClick={() => navigate(backTarget)} className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/72 px-4 py-2 text-sm font-medium text-slate-500 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.4)] transition hover:bg-white dark:border-slate-700/60 dark:bg-slate-900/80 dark:text-slate-300 dark:hover:bg-slate-800"><ChevronLeft size={15} /> {backLabel}</button>
      <div className="rounded-[28px] border border-white/70 bg-white/86 p-6 text-center shadow-[0_24px_70px_-46px_rgba(15,23,42,0.45)] dark:border-slate-700/60 dark:bg-slate-900/80">
        ...
        <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">The selected submission may have been removed or is no longer available.</p>
      </div>
    </PortalPage>;
  }
  ...
  return <PortalPage className="max-w-5xl space-y-6">
    ...
    <div className="space-y-5 rounded-[28px] border border-white/70 bg-white/86 p-6 shadow-[0_24px_70px_-46px_rgba(15,23,42,0.45)] dark:border-slate-700/60 dark:bg-slate-900/80">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Submission Detail</p>
      ...
      <div className="space-y-5">
        <div className="rounded-[22px] border border-slate-100 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/80">
          <div className="flex items-start gap-3">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-500/15"><FileText size={14} className="text-blue-700 dark:text-blue-200" /></div>
            <div className="flex-1">
              <p className="mb-1 text-xs font-bold text-slate-700 dark:text-slate-200">Teacher Feedback</p>
              {submission.feedback ? <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">{submission.feedback}</p> : <p className="text-sm italic text-slate-400 dark:text-slate-500">No feedback yet. Check back after your teacher reviews this submission.</p>}
            </div>
          </div>
        </div>
        ...
        {submission.type === "Group" && (
          <div className="rounded-[22px] border border-slate-100 bg-slate-50 p-4 dark:border-slate-700/60 dark:bg-slate-800/80">
            <div className="flex items-start gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-teal-50 dark:bg-teal-500/15"><Users size={14} className="text-teal-700 dark:text-teal-200" /></div>
              <div className="flex-1">
                ...
                {!!submission.members?.length && <div className="mt-3 space-y-1"><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-500">Members</p>{submission.members.map((member) => <div key={member} className="text-sm text-slate-500 dark:text-slate-400">• {member}</div>)}</div>}
              </div>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr,0.8fr]">
        <div className="rounded-[22px] border border-slate-100 p-4 dark:border-slate-700/60">
          <p className="mb-3 text-xs font-bold text-slate-700 dark:text-slate-200">Submitted Files</p>
          {linkedFiles.length > 0 ? <div className="space-y-2">{linkedFiles.map((file) => <div key={`${file.label}-${file.href}`} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2.5 dark:bg-slate-800/80"><span className="break-all text-sm text-slate-600 dark:text-slate-300">{file.label}</span>{file.href ? <a href={file.href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-blue-700 hover:underline dark:text-blue-200"><Download size={13} /> Download</a> : <span className="text-xs text-slate-400 dark:text-slate-500">Attached</span>}</div>)}</div> : <p className="text-sm text-slate-400 dark:text-slate-500">No uploaded files were attached to this submission.</p>}
        </div>

        <div className="space-y-4 rounded-[22px] border border-slate-100 p-4 dark:border-slate-700/60">
          <div>
            ...
            <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
              ...
              <p><span className="font-semibold text-slate-800 dark:text-slate-100">Grade:</span> {submission.grade !== "—" ? `${submission.grade}/100` : "Pending"}</p>
          ...
          {externalLinks.length > 0 && <div><p className="mb-2 text-xs font-bold text-slate-700 dark:text-slate-200">External Links</p><div className="space-y-2">{externalLinks.map((link) => <a key={link} href={link} target="_blank" rel="noreferrer" className="flex items-center gap-2 break-all text-sm text-blue-700 hover:underline dark:text-blue-200"><ExternalLink size={14} /> {link}</a>)}</div></div>}

          <div className="mt-2 flex flex-wrap gap-2">
            <button onClick={() => navigate(backTarget.includes("/student/submissions") ? backTarget : "/student/submissions")} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-700/60 dark:text-slate-200 dark:hover:bg-slate-800">All Submissions</button>
