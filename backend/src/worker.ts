...
bootstrapWorker().catch((error) => {
  ...
  if (/Database connection failed|Can't reach database server|P1001|ECONNREFUSED/i.test(message)) {
    logger.error('Worker startup failed because PostgreSQL is not reachable or DATABASE_URL points at the wrong database. Run npm run backend:doctor, then npm run prepare:local.');
  }
  process.exit(1);
});
