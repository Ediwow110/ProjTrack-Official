...
export default function UsersPage() {
      ...
      const pickValue = (row: AdminUserRecord) => {
        switch (sortState.columnKey) {
          case "displayIdentifier":
            return String(row.displayIdentifier || "").toLowerCase();
          case "name":
  ...
  async function handleCreateAdmin() {
    ...
    setFeedback(null);
    try {
      const result = await adminService.createAdmin({ ...createForm, email });
      let message = "Admin account created without sending activation email.";
      if (createForm.sendActivationEmail !== false) {
        const mailJobId = assertConfirmedMailJob(result, "admin activation email");
        message = `Admin account created and activation MailJob queued (${mailJobId}). Open Mail Jobs to watch delivery.`;
      }
      setCreateOpen(false);
  ...
  async function runMailUserAction(
    action: () => Promise<unknown>,
    successMessage: (mailJobId: string) => string,
    actionLabel: string,
  ) {
    if (actionState.busy) return;
    setActionState({ busy: true, error: null });
    ...
    try {
      ...
      const mailJobId = assertConfirmedMailJob(result as any, actionLabel);
      setFeedback({ tone: "success", message: successMessage(mailJobId) });
      setDeleteTarget(null);
      setDeleteConfirmation("");
      setSelectedUser(null);
      await reload();
      ...
      const message = err instanceof Error ? err.message : "The requested mail action could not be completed.";
      setActionState({ busy: false, error: message });
    }
  }
    ...
    <RoleListShell
      ...
      actions={(
        <>
          ...
          <Button
            ...
            className="bg-white dark:bg-slate-900/85 text-slate-900 dark:text-slate-100 hover:bg-slate-100"
          >
            <ShieldPlus size={14} />
            Add Admin
          </Button>
      ...
      toolbar={(
        <FilterToolbar
          ...
          searchPlaceholder="Search by name, email, student ID, or employee ID"
          primaryFilters={(
            ...
            <div
              className={`flex items-start gap-2 rounded-2xl border px-4 py-3 text-sm font-medium ${
                feedback.tone === "success"
                  ? "border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 dark:border-emerald-500/40 dark:bg-emerald-500/10 dark:text-emerald-200"
                  : "border-rose-200 dark:border-rose-500/30 bg-rose-50 dark:bg-rose-500/15 text-rose-700 dark:text-rose-300 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-200"
              }`}
            >
              <BootstrapIcon name={feedback.tone === "success" ? "check-circle-fill" : "x-circle-fill"} tone={feedback.tone === "success" ? "success" : "danger"} className="mt-0.5 shrink-0" />
              <span>{feedback.message}</span>
            </div>
          ) : null}
          {actionState.error ? (
            <div className="flex items-start gap-2 rounded-2xl border border-rose-200 dark:border-rose-500/30 bg-rose-50 dark:bg-rose-500/15 px-4 py-3 text-sm font-medium text-rose-700 dark:text-rose-300 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-200">
              <BootstrapIcon name="x-circle-fill" tone="danger" className="mt-0.5 shrink-0" />
    ...
    >
      <UsersTable
        ...
        onResendActivation={(user) =>
          runMailUserAction(() => adminService.resendUserActivation(user.id), (mailJobId) => `Activation MailJob queued for ${user.email} (${mailJobId}). Open Mail Jobs to watch delivery.`, "user activation email")
        }
        onDelete={(user) => {
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="sm:col-span-2 flex items-center gap-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/70 px-4 py-3 text-sm text-slate-700 dark:text-slate-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
            <input
              type="checkbox"
        ...
        {selectedUser ? (
          <div className="space-y-4 text-sm text-slate-600 dark:text-slate-300">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <DetailRow label={selectedUser.identifierLabel || "Student/Employee ID"} value={selectedUser.displayIdentifier || "—"} />
              <DetailRow label="Role" value={roleLabel(selectedUser.role)} />
              <DetailRow label="Status" value={selectedUser.status} />
              <DetailRow label="Profile ID" value={selectedUser.profileId || selectedUser.profileLabel || "—"} />
...
function DetailRow({
  ...
  return (
    <div className="space-y-1">
      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-300">
        {label}
      </p>
      <div className="text-sm text-slate-700 dark:text-slate-200 dark:text-slate-100">{value}</div>
    </div>
  );
}
