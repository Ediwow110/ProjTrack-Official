...
Before any production deploy, run and record the result of the following gate:

1. `npm run typecheck`
2. `npm run build`
3. `npm run check:click-targets`
4. `npm run check:theme`
5. Run light/dark QA on critical admin, student, and teacher pages.
6. Verify long modals and drawers at small laptop height, including 1366x768.
7. Verify Mail Jobs live updates and confirm every email action returns a MailJob ID before showing success.
8. Verify no API errors appear in the browser console.
9. Verify no success message appears without backend confirmation.
10. Verify backend tests when backend code changed.
11. Run production-hardening checks before release.

## Future UI Change Rules

- Every new card, table, form, dropdown, drawer, and modal must use shared portal theme classes or explicit light/dark variants.
- Every changed page must be checked in light and dark mode.
- Do not add page-level `bg-white`, `text-black`, or hardcoded slate/gray surfaces without a dark variant or shared token.
- Long modals and drawers must use a flex column shell with a bounded scrollable body and reachable action footer.
- Any action that sends email must return a confirmed MailJob ID before the UI shows queued or sent success.
- Any destructive action must show confirmation, archive/undo protection, or equivalent safe guardrails.
- Any backend failure must produce safe visible/admin diagnostics, not silent success.
- Any new admin endpoint must have auth/role protection and tests.
- Any new mail flow must create a MailJob row and be visible in Mail Jobs.

## 2026-04-28 Theme and Modal Hardening Addendum

- Added `npm run check:theme` to catch high-risk hardcoded page-level color utilities on protected admin pages and shared surface components.
- Hardened the shared app modal shell so long content scrolls inside the modal body while the header and footer remain reachable.
- Updated Seed Data Cleanup copy and backend result metadata to distinguish preview, blocked, and executed states.
- Updated Mail Jobs test mail flow so the UI requires a returned MailJob ID before displaying success.

## 2026-04-28 Silent Bug and Bootstrap Icon Addendum

- Added shared frontend mail-action guards so user, teacher, student, and admin mail actions must verify a returned `mailJobId`/`jobId` before showing success.
- Classroom notification flows now report in-app notification count, confirmed email job count, and safe warnings instead of always saying email jobs were queued.
- Added shared Bootstrap-style inline SVG status icons with `success`, `danger`, `warning`, `info`, `primary`, `secondary`, and `muted` tones for consistent semantic UI feedback.
- Seed cleanup storage removal now passes an admin actor context and reports stored submission file removal count in the execution result.

## Dark Mode and Silent-Failure Follow-up Gate

Latest hardening pass adds explicit dark-mode variants to legacy portal pages and a scoped portal compatibility layer for older alpha utilities. Before release, verify that Groups, Reports, Release Status, Deployment Checklist, Profile, student pages, and teacher pages have readable cards, tables, inputs, and chart labels in dark mode.

Silent-failure requirements now include:

- Broadcast/admin email queueing must confirm a MailJob before incrementing queued counts.
- Seed cleanup deletes database records inside the guarded transaction first, then removes storage objects; storage cleanup failures are returned as warnings instead of silently hiding orphaned files.
- Backup retention returns partial-failure details instead of reporting unconditional success.
- Profile avatar preview loading failures surface a safe visible page error instead of disappearing silently.
