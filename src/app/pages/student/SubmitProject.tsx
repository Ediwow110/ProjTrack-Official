...
export default function StudentSubmitProject() {
  ...
  if (submittedRecordId) {
    return <div className="p-6 max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <p className="text-slate-500 text-sm mb-6">Your submission has been saved and is ready for teacher review.</p>
  ...
  return <div className="p-6 max-w-4xl mx-auto space-y-6">
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="student-submit-subject" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Subject</label>
          <select id="student-submit-subject" value={subject} onChange={(e) => { setSubject(e.target.value); setActivityId(""); }} className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400">
            <option value="">Select subject</option>
            {subjects.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="student-submit-activity" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Activity</label>
          <select id="student-submit-activity" value={activityId} onChange={(e) => setActivityId(e.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400" disabled={!subject}>
      ...
      {selectedActivity && <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
        ...
        {isLocked && <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-700 inline-flex items-center gap-2"><Lock size={14} /> {isEditingExistingSubmission && !isEditableExistingSubmission ? "This submission can no longer be edited from the student flow." : "This activity is not currently open for editing or submission."}</div>}
      </div>}

      <div>
        <label htmlFor="student-submit-title" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Submission Title</label>
        <input id="student-submit-title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter a clear submission title" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400" required />
      </div>

      <div>
        <label htmlFor="student-submit-description" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Description</label>
        <textarea id="student-submit-description" value={desc} onChange={(e) => setDesc(e.target.value)} rows={5} placeholder="Describe the work you are submitting" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400 resize-none" required />
      </div>

      <div>
        <label htmlFor="student-submit-notes" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Notes</label>
        <textarea id="student-submit-notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Add optional notes for your teacher" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400 resize-none" />
      </div>

      <div>
        <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">External Links</label>
        <div className="space-y-2">
          {links.map((link, index) => <div key={index} className="flex items-center gap-2"><input value={link} onChange={(e) => setLinks((current) => current.map((item, i) => i === index ? e.target.value : item))} placeholder="https://..." className="flex-1 rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400" aria-label={`External link ${index + 1}`} />{links.length > 1 && <button type="button" onClick={() => setLinks((current) => current.filter((_, i) => i !== index))} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-50" aria-label={`Remove external link ${index + 1}`}><X size={14} /></button>}</div>)}
          <button type="button" onClick={() => setLinks((current) => [...current, ""])} className="text-sm font-semibold text-blue-700 hover:underline" aria-label="Add another submission link">Add another link</button>
        </div>
      </div>

      <div>
        <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Upload File</label>
        <label htmlFor="student-submit-file" className="flex items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-sm text-slate-500 cursor-pointer hover:border-blue-300 hover:bg-blue-50/50">
          <Upload size={16} />
