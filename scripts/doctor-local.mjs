...
async function main() {
  ...
  console.log(`- Database: ${database.ok ? database.detail : JSON.stringify(database)}`);
  console.log(`- Storage: ${storage.ok ? storage.detail : JSON.stringify(storage)}`);
  console.log(`- Mail: ${mail.ok ? mail.detail : JSON.stringify(mail)}`);
  console.log(`- Ready: ${ready.ok ? "ready" : JSON.stringify(ready)}`);

  if (frontendStatus !== "ok" || !backendLive.ok || !database.ok) {
    process.exit(1);
  }
}
