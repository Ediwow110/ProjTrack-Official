  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200 shadow-xl p-8 space-y-6">
        <Link to={backTarget} className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-[#346069]">
          <ArrowLeft size={15} /> Back to login
        </Link>
        <ProjTrackLogo role={logoRole} subtitle="Password Assistance" className="max-w-full" />
        <div>
          <h1 className="text-2xl font-bold text-[#24203B]">Forgot password</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button disabled={!email || submitting} type="submit" className="w-full rounded-2xl px-4 py-3.5 bg-[#346069] text-white text-sm font-semibold hover:bg-[#24203B] disabled:opacity-60 transition-colors">
