      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <div
              key={index}
              className="h-[280px] animate-pulse rounded-[28px] border border-white/70 bg-white/85"
            />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          {filtered.map((subject, index) => (
            <div
              ...
              className="group relative overflow-hidden rounded-[30px] border border-white/70 bg-white/88 p-6 shadow-[0_24px_70px_-44px_rgba(15,23,42,0.42)] transition duration-200 hover:-translate-y-1 hover:border-teal-200 hover:shadow-[0_30px_75px_-40px_rgba(13,148,136,0.4)]"
            >
              ...
              <div className="relative">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex gap-4">
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[22px] bg-teal-100 text-teal-700 shadow-inner shadow-white">
                      <BookOpen size={22} strokeWidth={1.8} />
                    </div>
                    <div className="min-w-0">
                      <span className="rounded-full bg-teal-100 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-teal-700">
                        {subject.code}
                      </span>
                      <p className="mt-3 font-display text-2xl font-semibold tracking-[-0.04em] text-slate-900">
                        {subject.name}
                      </p>
                      <p className="mt-1 text-sm text-slate-500">{subject.section}</p>
                    </div>
                  </div>
                  <div className="mt-1 rounded-2xl bg-slate-100 p-2 text-slate-400 transition group-hover:bg-teal-100 group-hover:text-teal-700">
                  ...
                  >
                    ...
                    <p
                      className={`mt-3 text-2xl font-semibold tracking-[-0.04em] ${
                        subject.pending > 5 ? "text-amber-700" : "text-slate-900"
                      }`}
                    >
                      {subject.pending}
                    </p>
